...
import {
	...
	getFileType,
	SupportedFileType,
} from "../utils/file/file-type-detector";
...
import { RebuildProgressManager } from "./rebuild-progress-manager";
import { OnCompletionManager } from "./completion-manager";
...
export class TaskManager extends Component {
			...
			if (this.workerManager && filesToProcess.length > 0) {
				try {
					...
					let enhancedProjectData:
						| import("../dataflow/workers/task-index-message").EnhancedProjectData
						| undefined;
	...
	private async processFileWithoutEvents(
		file: TFile,
		enhancedProjectData?: import("../dataflow/workers/task-index-message").EnhancedProjectData
	): Promise<Task[]> {
		if (!this.workerManager) {
