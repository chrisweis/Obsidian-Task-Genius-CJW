...
export class RightDetailView extends ItemView {
  ...
  getViewType(): string { return TG_RIGHT_DETAIL_VIEW_TYPE; }
  getDisplayText(): string { return "Task Genius Details"; }
  getIcon(): string { return "layout-sidebar-right"; }
  ...
  async onOpen() {
    ...
    this.registerEvent(
      onTaskSelected(this.app, async (payload: TaskSelectionPayload) => {
        ...
        if (!payload.taskId) {
          this.details.showTaskDetails(null as any);
          return;
        }
        try {
